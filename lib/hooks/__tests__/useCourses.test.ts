...
describe('useCourse', () => {
  it('returns 18 blank holes for a new course without hitting the network', () => {
    ...
    expect(result.current.holes[0]).toEqual({
      hole_number: 1,
      par: null,
      length_meters: null,
      stroke_index: null,
  ...
  it('loads an existing course and merges holes by hole_number', async () => {
    const courseBuilder = createQueryBuilderMock({
      data: { id: 'abc', name: 'Pebble', hole_count: 18, course_rating: 72.5, slope_rating: 130 },
      error: null,
    });
    const holesBuilder = createQueryBuilderMock({
      data: [{ hole_number: 1, par: 4, length_meters: 380, stroke_index: 7 }],
      error: null,
    });
    ...
    expect(result.current.holes[1]).toEqual({
      hole_number: 2,
      par: null,
      length_meters: null,
      stroke_index: null,
...
describe('saveCourse', () => {
  ...
  it('throws CourseValidationError when name is empty', async () => {
    await expect(
      saveCourse({ name: '', hole_count: 18, course_rating: 72.5, slope_rating: 130, holes: validHoles })
  ...
  it('throws CourseValidationError when a hole is missing par', async () => {
    ...
    await expect(
      saveCourse({ name: 'Test', hole_count: 18, course_rating: 72.5, slope_rating: 130, holes })
    ).rejects.toThrow(CourseValidationError);
  });
